...
export default function Home() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const nextOrders = (orderRows ?? []).map((o) => ({ dbId:o.id, id: `PED-${String(1000 + o.id).padStart(4, "0")}`, customer: o.customer, items: (o.order_items ?? []).map((item: { id:number; product_id:number; quantity:number; reserved_quantity:number; products:{name:string;sku:string} | null }) => ({ id:item.id, productId:item.product_id, productName:item.products?.name ?? "Producto", sku:item.products?.sku ?? "", quantity:item.quantity, reserved:item.reserved_quantity })), date: new Date(o.created_at).toLocaleString("es-MX", { dateStyle: "short", timeStyle: "short" }), status: o.status as Order["status"] }));
  ...
  const nav = ["Resumen", "Productos", "Pedidos", "Compras"];
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="content">
        ...
        {tab === "Pedidos" && <section className="panel table-panel"><div className="toolbar"><h2>Todos los pedidos</h2><span>{orders.length} registros</span></div><div className="table-wrap"><table><thead><tr><th>Folio</th><th>Cliente</th><th>Lista solicitada</th><th>Por abastecer</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{orders.map((o) => <tr key={o.id}><td><b>{o.id}</b><small>{o.date}</small></td><td>{o.customer}</td><td>{o.items.map((item) => <small key={item.id}><b>{item.quantity}×</b> {item.productName} <em>({item.reserved} apartados)</em></small>)}</td><td>{o.items.filter((item) => item.reserved < item.quantity).map((item) => <small className="shortage" key={item.id}>⚠ {item.productName}: faltan {item.quantity - item.reserved}</small>)}{["Listo","Entregado"].includes(o.status) && <span className="status ok">Sin faltantes</span>}</td><td><span className={`status ${["Listo","Entregado"].includes(o.status) ? "ok" : o.status === "Parcial" ? "warning" : o.status === "Cancelado" ? "neutral" : "danger"}`}>{o.status}</span></td><td><div className="row-actions">{o.status === "Listo" && <button className="icon-action edit" onClick={() => void deliverOrder(o.dbId)}>Entregar</button>}{!["Entregado","Cancelado"].includes(o.status) && <button className="icon-action remove" onClick={() => void cancelOrder(o.dbId)}>Cancelar</button>}</div></td></tr>)}</tbody></table></div></section>}
