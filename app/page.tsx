...
export default function Home() {
  ...
  async function createOrder(e: FormEvent<HTMLFormElement>) {
    e.preventDefault(); const fd = new FormData(e.currentTarget);
    if (!orderLines.length || orderLines.some((line) => line.quantity < 1)) { flash("Agrega al menos un producto válido"); return; }
    try { const { error } = await supabase.rpc("create_order", { p_customer: String(fd.get("customer")), p_items: orderLines.map((line) => ({ product_id: line.productId, quantity: line.quantity })) }); if (error) throw error; await loadData(); setModal(null); flash("Pedido guardado; el stock disponible quedó apartado"); }
  ...
  return <div className="app-shell">
    ...
    {modal && <div className="modal-backdrop" onMouseDown={() => setModal(null)}><div className="modal" onMouseDown={(e) => e.stopPropagation()}><button className="modal-close" onClick={() => setModal(null)}>×</button>
      ...
      {modal === "order" && <form onSubmit={createOrder}><p className="eyebrow">NUEVO PEDIDO</p><h2>Lista de productos</h2><p>Se apartará el stock disponible. Los faltantes quedarán marcados para reabastecimiento.</p><label>Cliente<input name="customer" required placeholder="Nombre del cliente" /></label><div className="order-lines">{orderLines.map((line, index) => { const item = productById(line.productId); const shortage = Math.max(0, line.quantity - (item?.stock ?? 0)); return <div className="order-line" key={index}><label>Producto<select value={line.productId} onChange={(e) => setOrderLines((all) => all.map((row,i) => i === index ? {...row,productId:Number(e.target.value)} : row))}>{products.map((p) => <option key={p.id} value={p.id}>{p.name} · {p.stock} disponibles</option>)}</select></label><label>Cantidad<input min="1" type="number" value={line.quantity} onChange={(e) => setOrderLines((all) => all.map((row,i) => i === index ? {...row,quantity:Number(e.target.value)} : row))} /></label>{orderLines.length > 1 && <button type="button" className="remove-line" onClick={() => setOrderLines((all) => all.filter((_,i) => i !== index))}>×</button>}{shortage > 0 && <small className="line-alert">⚠ Faltan {shortage}; se agregará a abastecimiento</small>}</div>})}</div><button type="button" className="add-line" onClick={() => setOrderLines((all) => [...all,{productId:products[0].id,quantity:1}])}>＋ Agregar otro producto</button><button className="primary submit">Guardar pedido</button></form>}
